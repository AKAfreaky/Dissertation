...
\abstract
Based on the work of Simon \citeasnoun{davies2012}, this dissertation takes the AI agent he developed for the video game StarCraft: Brood war and extends it to work for other races/factions in the game.

Part of the development included a survey of recent AI techniques that have been applied to StarCraft, that could be useful for future work on this project.

This document details the design and implementation processes employed during development, including the issues encountered and how they were overcome. It offers some ideas of improvements to the Behaviour Oriented Design methodology, and identifies areas for further improvements to the project.

The software resulting from this project can play a full game as any of the races/factions in StarCraft. It also provides the tools to create more varied and possibly more successful agents.
\newpage
...
I would like to thank my supervisor Dr. Bryson for the support given through out this project.

I would also like to thank Simon Davies for his work and the readability of his code.
\newpage
...
This dissertation builds upon the work done by Simon \citeasnoun{davies2012} where he created an AI agent for the video game StarCraft: Brood War. In the game of StarCraft, there are three ``races'' or factions which have different strategies and capabilities. The agent Davies created could only play as a single race and the aim of this dissertation was to extend Davies' work to the other races in the game.
...
The tests used the same metrics that \citeasnoun{davies2012} used. This allows for a more meaningful comparison with Davies's work. Davies' used two metrics, the outcome of the match and the percentage difference between the final scores of both players. Wether a player has won or lost is decided by the game, as is each player's score. The percentage difference between the final scores was calculated as the BOD agent's score minus the enemy agent's score, then divided by the BOD agent's score and multiplied by 100.
...
This project has extended and generalised the work of \citeasnoun{davies2012} from being able to control a single race in StarCraft, to being able to control any race in the game. The development process followed the BOD development methodology and architecture, using POSH action selection to call a variety of developed behaviours. These behaviours were linked to the game of StarCraft via the Java Native Interface to the  Brood War API \cite{JNIBWAPI}. The details and rationale behind the development of the behaviours and plans have been discussed throughout this dissertation. Remaining issues and areas for future development have been identified and discussed.
...
This appendix covers the documentation for running the agents, which is also available in the readme packaged with the code. It is adapted from the readme distributed by \citeasnoun{davies2012}.
\section{Readme}
------------------------\\
...
\lstinputlisting[basicstyle=\scriptsize,tabsize=4,language=Java,firstline=29,lastline=72]{ProductionManager.java}
...
\lstinputlisting[basicstyle=\scriptsize,tabsize=4,language=Java,firstline=75,lastline=102]{ProductionManager.java}
...
\lstinputlisting[basicstyle=\scriptsize,tabsize=4,language=Java,firstline=11]{UpgradeManager.java}
