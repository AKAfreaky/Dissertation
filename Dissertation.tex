...
The task of starting to gather gas is a two step process. The first a building needs to be built on the gas resource nodes (called an extractor). Once this building has completed, workers must be moved from other tasks to begin harvesting the gas. The original POSH plan (see listing \ref{oldGasPOSH} had both these steps in a competence. Because of bugs encountered from upgrading the external APIs (see section \ref{APIIssue}), the second step of the competence would often not work, and it would take some time to start gathering gas.

The solution found for this is to move the second step directly into the behaviour library. It will never be the case that a gas extraction building is constructed without wanting to send workers to it. There is an event when ever a unit is completed, so the completion of the extraction building.
